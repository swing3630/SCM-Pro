                ...
                <el-table :data="tabelList" stripe size="mini" @expand-change="expandRow" :row-key='getRowKeys'
                    ...
                    <el-table-column width="100px" label="操作">
                        <template slot-scope="scope">
                            <el-button type="danger" @click="instockPro(scope.row,scope.$index)" size="mini">付款登记</el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //获取可付款
            getInstock() {
                this.$axios.get(`/api/main/purchase/pomain/show?type=4&payType=${this.payType}&page=1`).then(resp => {
            ...
            current_change(current) {
                ...
                this.$axios.get(`/api/main/purchase/pomain/show?type=4&payType=${this.payType}&page=${this.currentPage}`).then(resp => {
                    if (resp.data.status == 1) {
                        this.$message({
                            message: '会话过期',
                            type: "error"
            ...
            responseFun(row,a,index) {
                this.$axios.get(`/api/main/finance/pay?poId=${row.poId}&type=${a}`).then(res => {
