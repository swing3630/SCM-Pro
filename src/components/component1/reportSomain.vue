<template>
    <div>
        ...
        <el-table :data="tableList" stripe>
            <el-table-column label="销售单总数" prop="totalnum"></el-table-column>
    ...
    export default {
        data() {
            return {
                searchDate: '',
                tableList: [
                    {totalnum:'',endtotalnum:'',sototal:'',totalpay:''}
                ],
                secondList: [],
                currentPage: 1,
                total: 0,
        ...
        methods: {
            querysomain() {
                this.$axios.get(`/api/main/report/somain/main?time=${this.searchDate}&page=${this.currentPage}`).then(res => {
                    ...
                    this.tableList[0].sototal = (res.data.sototal).toFixed(2)
            ...
            lastMoneyFn(payType, status, index) {
                if (status == 4 || status == 3) {
                    ...
                    return this.secondList[index].lastMoney = this.secondList[index].soTotal - this.secondList[index].prePayFee
                } else if (status == 1) {
                    return this.secondList[index].lastMoney = this.secondList[index].soTotal
                } else if (status == 2) {
                    if (payType == 1) {
                        return this.secondList[index].lastMoney = this.secondList[index].soTotal
                    }else if(payType == 2){
